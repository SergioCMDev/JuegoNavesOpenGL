...
Window* Window::Instance(const int widht, const int height, Camera camera ) {
	if (_window == nullptr) {
		cout << "NULL"<<endl;
		_window = new Window(widht, height, camera);
	}
	return _window;
}
...
Window::Window(const int widht, const int height) {
	...
	glfwSetCursorPosCallback(_glfwWindow, this->OnMouse);
...
void Window::HandlerInput() {
	...
	if (glfwGetKey(_glfwWindow, GLFW_KEY_0) == GLFW_PRESS) {
		_buttonMoreShiny = true;
	}
	else
	if (glfwGetKey(_glfwWindow, GLFW_KEY_0) == GLFW_RELEASE) {
		_buttonMoreShiny = false;
	}

	if (glfwGetKey(_glfwWindow, GLFW_KEY_1) == GLFW_PRESS) {
		_buttonLessShiny = true;
	}
	else
	if (glfwGetKey(_glfwWindow, GLFW_KEY_1) == GLFW_RELEASE) {
